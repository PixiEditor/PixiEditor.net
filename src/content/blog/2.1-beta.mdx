...
After months of work, I can finally show you exciting changes and new features. The long awaited Node Based Brush Engine is here, along with Smart Layers, Blackboard system, new Renderer and much more!


This is a **development** release, which means it may be buggy, there are missing features and there is a risk that files made in dev versions will not work as expected in next stable releases.
...
<video src="/videos/2.1-beta/paint.webm" loop muted autoPlay controls playsinline/>
...
![brush](assets/2.1-beta/brush.png)

**Brushes behave differently in Pixel Art and Painting toolset. Antialiasing is not automatically applied in the Pixel Art toolset. Some Brushes are custom and may still apply it.**
...
We've added 2 stabilizers: Time and distance based. They are available within pen tool settings in the viewport. 
...
PixiEditor's advanced Node Graph allows for exposing values to parent document, if they are embedded as a nested document (smart layer). Additionally, with new Blackboard feature, you can define inputs to your smart layers. 
...
As a response to a few important issues, I've decided to rewrite a lot of the core rendering components. There are plenty of new optimizations and characteristics. A few issues that the new renderer is trying to solve:
...
Please report any issues with rendering. If you're interested about this topic in more depth, check out [November Status](http://localhost:4321/blog/2025/10/02/september-status#new-renderer) post.
...
- Blackboard Variable Value: Accessing blackboard variables
...
- Expose Value: Exposes arbitrary value from the graph as an output, when nested into other document.
...
- Keyboard Info: Information about keyboard state, currently booleans about modifier keys.
...
You can now create your own toolsets and tools based on Brushes. Simply create a Brush that does what you want your tool to do, then create a config file to define your tool and toolset.
...
- Clipboard now should work much better and more stably. Linux bros, please test!
...
Some Brushes suffer from this greatly, I'm working on improving it, but for now, make sure to use the Render API which works better on your machine (sorry MacOS!)
