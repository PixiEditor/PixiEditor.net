...
This is an interesting example because Audacity is probably one of the most popular audio editing software known to man, outrunning other, proprietary ones. But the UI, oh boy, just look at that.
...
We've listed some of the new stuff in [previous devlog](/blog/2022/07/04/devlog2), however, I'll describe all of them in short.
...
Selection now allows scaling and rotating (with pivot!). Maybe one day we'll add skewing too.
...
Do you know those funny things that women put on their faces? Yeah, now imagine a woman's face is a layer.
Got it? Good. The whole face is visible. Now if you create a mask (or put it on the face) you can't see 
the face at all. Now when you draw on the mask, you are removing parts of it from the woman's face,
so you can see facial parts underneath. That's how layer masks work.

Below is a video example, except there is no woman and I'm masking the avocado.
